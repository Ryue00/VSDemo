...
void store() {//定义存储操作的方法；
	...
	if (count_count == 0) {
		aa[count_count] = temp;
		printf("x=%d,y=%d,t=%d,count_count=%d 第一次操作存入成功！\n", aa[count_count].x, aa[count_count].y, aa[count_count].t, count_count);
		count_count++;
	}
	if ((temp.x == aa[count_count - 1].x) && (temp.t - aa[count_count - 1].t < 120)) {//过滤同点时间间隔过短的点击
	}
	else {
		aa[count_count] = temp;
		printf("x=%d,y=%d,t=%d,count_count=%d 操作存入成功！\n", aa[count_count].x, aa[count_count].y, aa[count_count].t, count_count);
		count_count++;//记录最后一次操作时也进行了count++，所以总操作数为count；
	}
}
...
int fun_1() {//录制操作
	while (true) {//循环监控鼠标按键情况
		...
		if (KEY_DOWN(VK_ESCAPE)) {
			...
			out();
			cout << "-------------------------------------------------\n";
			printf("录制结束！共录制了%d个操作,耗时%d毫秒！\n", count_count, (aa[count_count - 1].t - aa[0].t));
